...
class SimpleParameter(Node):
    def __init__(self):
        ...
        self.declare_parameter("simple_string_param", "farhan")
    ...
    def paramChangeCallback(self, params):
        ...
        for param in params:
            if param.name == 'simple_int_param' and param.type_ == Parameter.Type.INTEGER:
                self.get_logger().info("Param simple_int_param changes! New value is %d " %param.value)
                result.successful = True
            if param.name == 'simpel_string_param' and param.type_ == Parameter.Type.STRING:
